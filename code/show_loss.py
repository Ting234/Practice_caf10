...
train_loss = np.loadtxt("../plot_data/temp.txt",delimiter=',',dtype=np.float32)
test_acc = np.loadtxt('../plot_data/test.txt', dtype=np.float32)
...
a[0][0].set_title('vgg16-trainAcc')
a[0][1].plot(itr, train_loss[:,0],label='second line',linewidth=1,color='b',marker='o',
             markerfacecolor='blue',markersize=1)
a[0][1].set_title('vgg16-trainloss')
a[1][0].plot(itr1, test_acc,label='second line',linewidth=1,color='b',marker='o',
             markerfacecolor='blue',markersize=1)
a[1][0].set_title('vgg16-teatAcc')
plt.suptitle('vgg16res_SGD_mish',fontsize = 20, color = 'red',backgroundcolor='yellow')
plt.savefig('../result/res_net_vgg16.jpg')
